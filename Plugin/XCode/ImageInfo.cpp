...
void ImageInfo::copyCroppedFrom(const ImageInfo* src,Vector2d pos,Vector2d sz,bool clamp)
{
    ...
    if(src->format==EPixel_I420)//in case of YUV format, only copy from grayscale data (Y channel)
    {
        EPixelFormat fmt=EPixel_LUMINANCE8;
        createData(sz, fmt);
        ...
        if(sz.y+pos.y>src->Size.y)wrapY=true;
